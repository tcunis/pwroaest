function varargout = pwreachvstep(f1,f2,phi,con,xbar,x,z,gbar,gamma,s0,s,si,sg,sj,L1,L2,roaopts)
...
if length(z) == 1
    ...
    if double(subs(phi,x,xbar)) <= 0
        % V1(xbar) <= gbar
        V1bar = subs(V1,x,xbar);
        sosconstr{3} = V1bar <= gbar;
    else
        % V2(xbar) <= gbar
        V2bar = subs(V2,x,xbar);
        sosconstr{4} = V2bar <= gbar;
    end
